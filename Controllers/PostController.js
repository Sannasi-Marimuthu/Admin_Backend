    ...
    if (type || ratetype) {
      ...
      if (type === "Filetype") {
        try {
            ...
            if (parts[0] === "utilityFiles" && parts.length === 4) {
              ...
              if (file.path.includes(path.join(uploadDir, "temp"))) {
                ...
                try {
                  ...
                  console.log(
                    `Moved PropertyImage from ${file.path} to ${destPath}`
                  );
                } catch (err) {
                  console.error(
                    `Error moving PropertyImage from ${file.path}:`,
                    err
                  );
                  throw err;
                }
              ...
              otherImageFiles["CoverImage"] =
                otherImageFiles["CoverImage"] || [];
              // Check if file is in temp or temp/CoverImage
              if (
                ...
                try {
                  fsSync.renameSync(file.path, destPath);
                  console.log(
                    `Moved CoverImage from ${file.path} to ${destPath}`
                  );
                } catch (err) {
                  console.error(
                    `Error moving CoverImage from ${file.path}:`,
                    err
                  );
                  throw err;
                }
          ...
          // Clean up temp folder (including subdirectories like temp/CoverImage)
              ...
              for (const file of files) {
                ...
                if (stat.isDirectory()) {
                  ...
                  try {
                    ...
                    console.log(
                      `Moved temp file from ${filePath} to ${destPath} as CoverImage`
                    );
                    otherImageFiles["CoverImage"] =
                      otherImageFiles["CoverImage"] || [];
                    ...
                    otherImageFiles["CoverImage"].push({
                      filename: relativePath,
                    });
                    tempFiles.push(destPath);
                  } catch (err) {
                    console.error(`Error moving temp file ${filePath}:`, err);
                  }
                }
              }
            };
          ...
          for (const key in req.body) {
            if (key.startsWith(`existingUtilityImages_${PropertyCode}_`)) {
              ...
              for (const filename of filenames) {
                ...
                try {
                  if (fsSync.existsSync(sourcePath)) {
                    ...
                    existingUtilityImages[index].push({
                      filename: relativePath,
                    });
                    tempFiles.push(destPath);
                  } else {
                  ...
                  console.error(
                    `Error copying utility image ${filename}:`,
                    err
                  );
                  throw err;
                }
                ...
                for (const filename of filenames) {
                  ...
                  try {
                    if (fsSync.existsSync(sourcePath)) {
                      ...
                      fsSync.copyFileSync(sourcePath, destPath);
                      existingRoomImages[roomCode].push({
                        filename: relativePath,
                      });
                      tempFiles.push(destPath);
                    } else {
              ...
              if (imageFields.includes(fieldName)) {
                ...
                console.log(
                  `Processing existing images for ${fieldName}:`,
                  filenames
                );
                ...
                for (const filename of filenames) {
                  ...
                  try {
                    if (fsSync.existsSync(sourcePath)) {
                      ...
                      fsSync.copyFileSync(sourcePath, destPath);
                      existingOtherImages[fieldName].push({
                        filename: relativePath,
                      });
                      tempFiles.push(destPath);
                    } else {
                    ...
                    console.error(
                      `Error copying ${fieldName} image ${filename}:`,
                      err
                    );
                    throw err;
                  }
            ...
            if (key.startsWith("removedUtilityImages_")) {
              ...
              for (const filename of removedUtilityImages[index]) {
                ...
                try {
                  ...
                  console.error(
                    `Failed to delete utility image ${filename}:`,
                    err
                  );
                }
              }
              ...
              if (roomCode === RoomCode) {
                removedRoomImages[roomCode] = parseJSON(
                  req.body[key],
                  [],
                  `removedRoomImages_${roomCode}`
                );
                for (const filename of removedRoomImages[roomCode]) {
                  ...
                  try {
                    ...
                    console.error(
                      `Failed to delete room image ${filename}:`,
                      err
                    );
                  }
                }
                ...
                for (const filename of removedOtherImages[fieldName]) {
                  ...
                  try {
                    if (fsSync.existsSync(filePath)) {
                      fsSync.unlinkSync(filePath);
                      console.log(
                        `Deleted removed ${fieldName} image: ${filename}`
                      );
                    }
                  } catch (err) {
                    console.error(
          ...
          if (existingProperty && existingProperty.Utilities) {
            updatedUtilities = existingProperty.Utilities.map(
              (utility, index) => {
                ...
                const existingPhotos = (utility.photos || [])
                  ...
                  .map((photo) => {
                    const matchingImage = (
                      existingUtilityImages[indexStr] || []
                    ).find((img) => img.filename === photo.filename);
          ...
          imageFields.forEach((field) => {
            const existingImages = existingFile?.[field]
              ? existingFile[field]
                  ...
                  .map((image) => {
                    const matchingImage = (
                      existingOtherImages[field] || []
                    ).find((img) => img.filename === image.filename);
                    return matchingImage
                      ? matchingImage.filename
                      : image.filename;
                  })
              : [];
          ...
          const updatedFileEntry = await File.findOneAndUpdate(
            query,
            updateData,
            {
              upsert: true,
