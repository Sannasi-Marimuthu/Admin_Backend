...
const Web = require("../Schema/Web/WebSchema.js");
...
router.post("/approval", upload.any(), async (req, res) => {
  ...
  try {
    ...
    const duplicateProperty = await Property.findOne({
      Propertyname: {
        $regex: `^${propertyData.Propertyname}$`,
        $options: "i",
      },
    });
    ...
    if (!propertyData.RatingId) {
      return res.status(400).json({ message: "RatingId is required" });
    }
    if (!propertyData.Yearofconstruction) {
      return res
        .status(400)
        .json({ message: "Yearofconstruction is required" });
    }
    if (!propertyData.Hotelmobile) {
      return res.status(400).json({ message: "Hotelmobile is required" });
    }
    if (!propertyData.Phonelist) {
      return res.status(400).json({ message: "Phonelist is required" });
    }
    if (!propertyData.Hotelemail) {
    ...
    if (!propertyData.IFSCCode) {
      return res.status(400).json({ message: "IFSCCode is required" });
    }
    if (!propertyData.Branch) {
      return res.status(400).json({ message: "Branch is required" });
    }
    if (!propertyData.BankName) {
      return res.status(400).json({ message: "BankName is required" });
    }
    if (!propertyData.PropertyOwner) {
      return res.status(400).json({ message: "PropertyOwner is required" });
    }
    if (!propertyData.DivisionType) {
      return res.status(400).json({ message: "DivisionType is required" });
    }
    if (
      propertyData.Noofrestaurants === undefined ||
      propertyData.Noofrestaurants === null
    ) {
      return res.status(400).json({ message: "Noofrestaurants is required" });
    }
    if (
      propertyData.Nooffloors === undefined ||
      propertyData.Nooffloors === null
    ) {
      return res.status(400).json({ message: "Nooffloors is required" });
    }
    if (
      propertyData.Noofrooms === undefined ||
      propertyData.Noofrooms === null
    ) {
      return res.status(400).json({ message: "Noofrooms is required" });
    }
    if (!propertyData.Customercare) {
      return res.status(400).json({ message: "Customercare is required" });
    }

    const flag = propertyData.Id;
    console.log("flag", flag);

    const UpdateWeb = await Web.findByIdAndUpdate(
      { _id: flag },
      { $set: { Approve: 1 } }, // Set Approve field to 1
    ...
    if (!roomsData.Totalrooms) {
      return res.status(400).json({ message: "Totalrooms is required" });
    }
    if (!roomsData.Roomview) {
      return res.status(400).json({ message: "Roomview is required" });
    }
    if (!roomsData.Bedview) {
      return res.status(400).json({ message: "Bedview is required" });
    }
    if (!roomsData.Roomsize) {
      return res.status(400).json({ message: "Roomsize is required" });
    }
    if (!roomsData.Measurement) {
      return res.status(400).json({ message: "Measurement is required" });
    }
    if (!roomsData.Basicadults) {
      return res.status(400).json({ message: "Basic Adults is required" });
    }
    if (!roomsData.Maxadults) {
      return res.status(400).json({ message: "Maximum Adults is required" });
    }
    if (!roomsData.Maxchildrens) {
      return res.status(400).json({ message: "Maximum Childrens is required" });
    }
    if (!roomsData.Maxoccupancy) {
      return res.status(400).json({ message: "Maximum Occupancy is required" });
    }
    if (!roomsData.SmokingPolicy) {
      return res.status(400).json({ message: "Smoking Policy is required" });
    }
    if (!roomsData.SmokingRooms) {
      return res.status(400).json({ message: "SmokingRooms is required" });
    }
    if (!roomsData.RoomAmenities) {
      return res.status(400).json({ message: "Room Amenities is required" });
    }
    if (!roomsData.RoomImages) {
      return res.status(400).json({ message: "Room Images is required" });
    }
    const roomRateData = parseJSON(req.body.roomRateData, [], "roomRateData");
    console.log("<<<<<<RATES>>>>>>>>>", roomRateData);
    if (!roomsData.RatePlan) {
      return res.status(400).json({ message: "Rate Plan is required" });
    }
    if (!roomsData.SingleTarrif) {
      return res.status(400).json({ message: "Single Tarrif is required" });
    }
    if (!roomsData.DoubleTarrif) {
      return res.status(400).json({ message: "Double Tarrif is required" });
    }
    const rateplanData = parseJSON(req.body.rateplanData, [], "rateplanData");
    console.log("<<<<<<RATEPLAN>>>>>>>>>", rateplanData);
